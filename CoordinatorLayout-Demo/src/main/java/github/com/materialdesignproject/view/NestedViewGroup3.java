...
public class NestedViewGroup3 extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
    ...
    public NestedViewGroup3(Context context) {
        this(context,null);
    }

    public NestedViewGroup3(Context context, AttributeSet attrs) {
        this(context, attrs,-1);
    }

    public NestedViewGroup3(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context,AttributeSet attrs){
        ...
        //获取自定义属性
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.NestedRefreshLoadMore);
        if(typedArray != null) {

            mRefreshEnabled = typedArray.getBoolean(R.styleable.NestedRefreshLoadMore_refreshenabled, true);
            mLoadMoreEnabled = typedArray.getBoolean(R.styleable.NestedRefreshLoadMore_loadMoreEnabled, true);

            if (typedArray.hasValue(R.styleable.NestedRefreshLoadMore_mode)) {
                int mode_int = typedArray.getInt(R.styleable.NestedRefreshLoadMore_mode, 0);
                mCurrentMode = Mode.values()[mode_int];
            }

            int headerResourceId = typedArray.getResourceId(R.styleable.NestedRefreshLoadMore_header, -1);
            if(headerResourceId > 0){
                View headerView = mLayoutInflater.inflate(headerResourceId, null);
                setRefreshHeaderView(headerView);
            }
            int footerResourceId = typedArray.getResourceId(R.styleable.NestedRefreshLoadMore_footer, -1);
    ...
    protected ViewGroup.LayoutParams generateDefaultLayoutParams() {
        return new NestedViewGroup3.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
    }
    /**
     * {@inheritDoc}
     */
    ...
    protected ViewGroup.LayoutParams generateLayoutParams(ViewGroup.LayoutParams p) {
        return new NestedViewGroup3.LayoutParams(p);
    }
    /**
     * {@inheritDoc}
     */
    ...
    public ViewGroup.LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new NestedViewGroup3.LayoutParams(getContext(), attrs);
    }
...
//        if(mTargetOffset > 0 &&  onCheckCanRefresh()){
    ...
    private class AutoScroller implements Runnable {
        ...
        public void run() {
            ...
            if (finish) {
                ...
                mmLastY = currY;
                NestedViewGroup3.this.autoScroll(yDiff);
                post(this);
            }
        }
        ...
        private void finish() {
            ...
            if (!mAbort) {
                NestedViewGroup3.this.autoScrollFinished();
            }
        }
